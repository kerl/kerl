...
```console
$ kerl emit-activate <release> <directory> [sh|bash|fish|csh]
```

Print the `activate` script, as generated by `kerl`.

**Note**: the output defaults to shell `sh` (also `bash`), as per `activate`. To emit
`activate.fish` or `activate.csh` equivalents set the last argument to `fish` or `csh`,
respectively.
